  ...
  describe Image do
    ...
    context "image url" do
      ...
      it "has different urls for each geometry string" do
        created_image.thumbnail('200x200').url.
                      should_not == created_image.thumbnail('200x201').url
      end

      it "uses right geometry when given a thumbnail name" do
        name, geometry = Images.user_image_sizes.first
        created_image.thumbnail(name).url.
                      should == created_image.thumbnail(geometry).url
      end
    end
    ...
    describe ".per_page" do
      context "dialog is true" do
        context "has_size_options is true" do
          it "returns image count specified by Images.pages_per_dialog_that_have_size_options option" do
            Image.per_page(true, true).
                  should == Images.pages_per_dialog_that_have_size_options
          end
        end

        context "has_size_options is false" do
          it "returns image count specified by Images.pages_per_dialog option" do
            Image.per_page(true).should == Images.pages_per_dialog
          end
        end
      end
      ...
      context "dialog is false" do
        it "returns image count specified by Images.pages_per_admin_index option" do
          Image.per_page.should == Images.pages_per_admin_index
        end
      end
    end
    ...
    describe ".user_image_sizes" do
      it "returns a hash" do
        Images.user_image_sizes.should be_a_kind_of(Hash)
      end
    end
    ...
    describe '#thumbnail_dimensions returns correctly with' do
      ...
      it 'nil' do
        created_alternate_image.thumbnail_dimensions(nil).
                                should == { :width => 376, :height => 184 }
      end

      it '225x255>' do
        created_alternate_image.thumbnail_dimensions('225x255>').
                                should == { :width => 225, :height => 110 }
      end
    end

    describe "validations" do
      let(:file) {
        Refinery.roots(:'refinery/images').join "spec/fixtures/beach.jpeg"
      }
      ...
      describe "too large #image" do
        before { Images.max_image_size = 0 }

        let!(:image) { Image.new :image => file }
        ...
        it "should contain an error message" do
          ...
          image.errors[:image].should == [
            "Image should be smaller than #{Images.max_image_size} bytes in size"
          ]
        end
      end
